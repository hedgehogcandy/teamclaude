...
| `proxy.managedClientAuth` | Let a client run against the pool **without a Claude login of its own** (default `false`). With it on, `teamclaude run` hands the launched CLI a facade bearer — `tc-managed-v1.<account id>.<HMAC-SHA256 over `proxy.apiKey`>` — that names an identity and carries no provider token; the proxy verifies the signature and injects the real account token upstream, so a stolen credential is worthless off this proxy. Eligible identities are enabled Anthropic OAuth accounts only (never Codex, API-key or third-party backends). Revocation is live: disabling the mode, removing the account, or rotating `proxy.apiKey` invalidates every issued credential at the next `POST /teamclaude/reload`. **Requires MITM** — `run --no-mitm` is refused, because base-URL routing leaves the CLI able to reach `api.anthropic.com` directly with no credential to use there. See [Pool-managed authentication](usage.md#pool-managed-authentication-no-personal-login) |
